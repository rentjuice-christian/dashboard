...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" class="form_select" id="form_submit" >
	<div class="align_right">
		<input type="hidden" value="supertagusageover" name="page" />
			...
			<?php
				if(count($sub_barcontent) != 0){
					foreach($sub_barcontent as $valuesub){
						if(!empty($valuesub->super_tag)){
				?>
						<option value="<?php echo $valuesub->super_tag; ?>" <?php if($tagUsageLabel == $valuesub->super_tag){ echo'selected'; }else{ echo''; }  ?>>
							<?php echo $valuesub->super_tag." (".$valuesub->super_tag_count.")" ?>
						</option>
				<?php
						}
					}
