<?php render('_header',array('title'=>'Data Entry Volume by Market'))?>
<?php

	$timeFrame = "";
	
	if(isset($_REQUEST['totallistings_time'])){
		$timeFrame = $_REQUEST['totallistings_time'];
	}
	else{
		$timeFrame = '2';
	}
...
$(function () {
    ...
    var chart = new Highcharts.Chart({
        ...
        xAxis: {
			...
			<?php		
				if(isset($_REQUEST['totallistings_time'])){
					if($_REQUEST['totallistings_time'] == 1){ echo",tickInterval:24 * 3600 * 1000	"; }
					if($_REQUEST['totallistings_time'] == 2){ echo",tickInterval:2 * 24 * 3600 * 1000	"; }
					if($_REQUEST['totallistings_time'] == 3){ echo",tickInterval:4 * 24 * 3600 * 1000	"; }
					if($_REQUEST['totallistings_time'] == 4){ echo",tickInterval:5 * 24 * 3600 * 1000	"; }
					if($_REQUEST['totallistings_time'] == 5){ echo",tickInterval:7 * 24 * 3600 * 1000	"; }
					if($_REQUEST['totallistings_time'] == 6){ echo",tickInterval:7 * 24 * 3600 * 1000	"; }
					if($_REQUEST['totallistings_time'] == 7){ echo",tickInterval:15 * 24 * 3600 * 1000	"; }
					if($_REQUEST['totallistings_time'] == 'alltime'){ echo",tickInterval:90 * 24 * 3600 * 1000	"; }
				}
				else{
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" class="form_select" id="form_submit" >
	<div class="align_right">
		<input type="hidden" value="totallistingsbymarket" name="page"  />
		<span>Select Time Frame: </span>
		<select name="totallistings_time" onchange="submit();" class="select_time">
